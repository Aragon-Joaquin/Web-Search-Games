...
export function SearchBar() {
  const { games, setGames, gamesRawData, setGamesRawData, getSessionCookie, allCookies } = useContext(GamesContext)
  ...
  async function getGames(evt) {
    ...
    const data = await FETCH_DATA({route: "/api/games",CLIENT_ID,access_token,searchParams})
  ...
  useEffect(() => {
    ...
    `;
          const data = await FETCH_DATA({route: `/api/${url[1]}`,CLIENT_ID,access_token,searchParams})
          return data;
        });
    ...
    if (gamesRawData != undefined) getGamesSubcategory({ gamesRaw: gamesRawData });
  }, [gamesRawData]);
  return (
    <>
      <section>
